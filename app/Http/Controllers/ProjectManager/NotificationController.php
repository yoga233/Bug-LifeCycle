...
class NotificationController extends Controller
{
    public function index(Request $request): View
    {
        ...
        return view('panel.project-manager.notifications', compact('notifications'));
    }
    ...
    public function markRead(Notification $notification, Request $request): RedirectResponse
    {
        ...
        if ($notification->related_id) {
            return redirect()->route('pm.issues.show', ['bug' => $notification->related_id, 'return' => url()->previous()]);
        }

        return redirect()->route('pm.notifications');
    }
