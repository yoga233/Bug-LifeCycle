...
@section('content')
    @php
        $unreadCount = $unreadCount ?? ($pmUnreadNotifications ?? 0);
    @endphp
    ...
    <x-pm.page-header
        ...
        title="Notifikasi"
        description="Lihat pembaruan bug terbaru secara ringkas dan fokus pada tindakan penting."
    />

    <section class="space-y-5">
        <div class="flex flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-base font-semibold text-slate-900">Daftar Notifikasi PM</h2>
                <p class="mt-0.5 text-sm text-slate-500">
                    {{ $unreadCount > 0 ? $unreadCount . ' notifikasi perlu dibaca' : 'Semua notifikasi sudah dibaca' }}
                </p>
            </div>

            <form method="POST" action="{{ route('pm.notifications.markAllRead') }}">
                @csrf
        ...
        <div class="overflow-hidden rounded-lg border border-slate-200 bg-white">
            @forelse($notifications as $n)
                @php
                    ...
                    [$label, $badgeStatus] = match($n->type) {
                        ...
                        'BugStatusChanged' => ['Status Diperbarui', 'In Progress'],
                        'BugCommented' => ['Dikomentari', 'Testing'],
                        'BugRejected' => ['Dikembalikan QA', 'Rejected'],
                ...
                <article class="border-b border-slate-200 px-4 py-4 last:border-b-0">
                    ...
                    <div class="mt-3 flex w-full flex-wrap items-center justify-end gap-2">
                        <form method="POST" action="{{ route('pm.notifications.read', $n) }}">
                            @csrf
                        ...
                        <form method="POST" action="{{ route('pm.notifications.destroy', $n) }}">
                            @csrf
                            @method('DELETE')
                            <x-pm.button
                                type="submit"
            ...
            @empty
                <div class="px-4 py-10 text-center">
                    ...
                    <p class="mt-1 text-xs text-slate-500">Notifikasi baru akan muncul saat ada report atau perubahan status bug.</p>
                    <x-pm.button-link href="{{ route('pm.notifications') }}" variant="secondary" size="sm" class="mt-3">
                        Refresh
                    </x-pm.button-link>
                </div>
            @endforelse
