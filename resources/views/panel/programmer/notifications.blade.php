...
@section('content')
        ...
        <div>
            <nav class="ui-text-caption" aria-label="Breadcrumb">
                <ol class="flex items-center gap-2">
                    <li>
                        <a href="{{ route('programmer.dashboard') }}" class="ui-text-link">
                            Programmer
                        </a>
                    </li>
            ...
            <p class="mt-2 ui-text-body">Lihat pembaruan bug yang perlu ditindaklanjuti secara ringkas.</p>
        </div>
    </div>
    ...
    <section class="space-y-5">
        <div class="flex flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-base font-semibold text-slate-900">Daftar Notifikasi Programmer</h2>
                <p class="mt-0.5 text-sm text-slate-500">
                    {{ $unreadCount > 0 ? $unreadCount . ' notifikasi menunggu dibaca' : 'Semua notifikasi sudah dibaca' }}
                </p>
            </div>

            <form method="POST" action="{{ route('programmer.notifications.markAllRead') }}">
                @csrf
        ...
        <div class="overflow-hidden rounded-lg border border-slate-200 bg-white">
            @forelse($notifications as $n)
                ...
                <article class="border-b border-slate-200 px-4 py-4 last:border-b-0">
                    ...
                    <div class="mt-3 flex w-full flex-wrap items-center justify-end gap-2">
                        <form method="POST" action="{{ route('programmer.notifications.read', $n) }}">
                            @csrf
                        ...
                        <form method="POST" action="{{ route('programmer.notifications.destroy', $n) }}">
                            @csrf
                            @method('DELETE')
                            <x-pm.button
                                type="submit"
            ...
            @empty
                <div class="px-4 py-10 text-center">
                    ...
                    <p class="mt-1 text-xs text-slate-500">Saat ada penugasan baru atau perubahan status, notifikasi akan muncul di sini.</p>
                    <x-pm.button-link href="{{ route('programmer.notifications') }}" variant="secondary" size="sm" class="mt-3">
                        Refresh
                    </x-pm.button-link>
                </div>
            @endforelse
