...
@php
    $baseClasses = 'inline-flex items-center justify-center gap-2 rounded-md font-medium transition focus:outline-none focus:ring-2 focus:ring-offset-2';
    ...
    $variantClasses = match ((string) $variant) {
        'primary' => $resolveSolidColorClasses($normalizedColor ?: 'blue'),
        'danger' => $resolveSolidColorClasses('red'),
        default => $resolveSoftColorClasses($normalizedColor ?: 'slate'),
    };

    $widthClasses = $block ? 'w-full' : '';
@endphp

<a {{ $attributes->merge(['class' => trim("{$baseClasses} {$sizeClasses} {$variantClasses} {$widthClasses}")]) }}>
    {{ $slot }}
</a>